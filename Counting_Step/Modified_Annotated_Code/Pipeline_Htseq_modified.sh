...
COUNTHT=/$WD/Counts_HTSeq       ## Example:/scratch/$MyID/PracticeRNAseq/Counts_StringTie
...
do
  ...
  ######################  Step 3b  Counting     ################
 ### HTSeq  which takes a GTF file with gene models and a sorted BAM file and counts for each gene how many reads map to itwas executed with default settings.
 ## -f to include sam/bam file used; Minaqual value represents the quality of the mapped data from the SAM/BAM files. The mapped data with a quality lower than the parameter given will be dropped while counting. 
 ##The max-reads-in-buffer determines the number of reads that can stay in the memory.
...
# List of variables
variables=("SRR13308380" "SRR13308381" "SRR13308382","SRR13308383" "SRR13308392")
...
do
    ...
    # Run HTSeq count
    python -m HTSeq.scripts.count -f bam --max-reads-in-buffer=35000000 -a 20 -t exon -i gene_id --additional-attr=gene_name --additional-attr=exon_number -c $CountHT/"$var"/$var"/"$var".csv -o $CountHT/"$var"/"$var".gtf $MAPD/"$var"_sorted.bam $REFD/{$REF}.gtf > $CountHT/"$var"/"$var".tab 
done
...
### copy the final results files (the count matricies that are .tab) to your home directory. 
cp *.tab ${RESULTSD}
